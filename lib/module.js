...
const { conditionMatches } = require('bare-module-resolve')
...
const hostConditions = ['bare', 'node', ...Bare.Addon.host.split('-')]
...
module.exports = exports = class Module {
  ...
  get protocol() {
    return this._loader._protocolFor(this.url)
  }

  get resolutions() {
    return this._loader._resolutions
  }
...
function pickCondition(entry, condition) {
  ...
  for (const [, target] of conditionMatches(entry, [condition, ...hostConditions])) {
    return pickCondition(target, condition)
  }
