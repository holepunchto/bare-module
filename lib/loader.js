...
function resolutionsFor(opts) {
  const { resolutions, cache } = opts

  if (resolutions === true) return defaultResolutions

  if (typeof resolutions === 'object' && resolutions !== null) return resolutions

  if (resolutions === false) return Object.create(null)
...
module.exports = class ModuleLoader {
  constructor(opts = {}) {
    const { protocol = null, builtins = null, defaultType = 0, imports = null, cache } = opts
    ...
    this._resolutions = resolutionsFor(opts)
    this._artifacts = { addons: [], assets: [] }
    this._bundles = []
    this._visited = new Set()
    this._pending = null

    setDefaultLoader(this)
  }
  ...
  async link(entry, source = null, opts = {}) {
    ...
    else await driveAsync(this._link(entry, source, opts), this._protocolFor(entry))
  ...
  linkSync(entry, source = null, opts = {}) {
    if (entry.protocol === 'builtin:') this._lookup(entry)
    else driveSync(this._link(entry, source, opts), this._protocolFor(entry))
  ...
  *_link(entry, source, opts) {
    ...
    const queue = [traverse.module(entry, source, attributes, this._artifacts, this._visited, opts)]
    const deferred = []
    ...
    while (queue.length > 0 || deferred.length > 0) {
      ...
      while (next.done !== true) {
        ...
        if (value.module) {
          const source = value.module.href === probed ? null : yield { module: value.module }
  ...
  _linkBundle(bundle) {
    ...
    driveSync(
      this._link(new URL(bundle.main), null, {
  ...
  _ingest(dependency) {
    ...
    if (Class === undefined) {
      throw errors.UNSUPPORTED_MODULE_TYPE(
  ...
  _instantiate() {
    ...
    do {
      ...
      for (const record of Object.values(this._cache)) record._instantiate()
    } while (Object.keys(this._cache).length > size)
  }
  ...
  _createRequire(referrer) {
    ...
    function require(specifier, opts = {}) {
      ...
      if (record === null) record = loader._requireSync(specifier, referrer.url, attributes)
  ...
  _requireSync(specifier, parentURL, attributes = null) {
    ...
    if (record === null) {
      this.linkSync(url, null, { attributes })
  ...
  _resolveSync(specifier, parentURL, condition) {
    ...
    while (next.done !== true) {
      ...
      if (value.package) {
        ...
        next = resolver.next(source === null ? null : JSON.parse(source.toString()))
      } else {
        const url = value.resolution

        if (url.protocol === 'builtin:' || this.get(url) !== null) {
          this._cacheResolution(parentURL, specifier, condition, url)

          return url
        }

        if (existsSync(protocol, url)) {
          const resolution = resolveSync(protocol, url)
  ...
  async _resolveAsync(specifier, parentURL, condition) {
    ...
    while (next.done !== true) {
      ...
      if (value.package) {
        ...
        const source = record ? record._source.bytes : await readAsync(protocol, value.package)
        ...
        if (await existsAsync(protocol, url)) {
          const resolution = await resolveAsync(protocol, url)
  ...
  _resolveArtifact(referrer, specifier, condition, parentURL = referrer.url) {
    if (condition === 'addon') {
      return Bare.Addon.resolve(specifier, parentURL, { referrer })
    }

    const url = parentURL === referrer.url ? referrer._resolve(specifier, 'asset') : null

    return url === null ? this._resolveSync(specifier, parentURL, 'asset') : url
  }
}

function driveSync(generator, protocol) {
  let next = generator.next()

  while (next.done !== true) {
    let value

    if ('module' in next.value) {
      value = readSync(protocol, next.value.module)
    } else if ('probe' in next.value) {
      value = existsSync(protocol, next.value.probe)
    } else if ('resolution' in next.value) {
      value = resolveSync(protocol, next.value.resolution)
    } else {
      value = listSync(protocol, next.value.prefix)
    }
...
async function driveAsync(generator, protocol) {
  let next = generator.next()
  ...
  while (next.done !== true) {
    ...
    if ('module' in next.value) {
      value = await readAsync(protocol, next.value.module)
    } else if ('probe' in next.value) {
      value = await existsAsync(protocol, next.value.probe)
    } else if ('resolution' in next.value) {
      value = await resolveAsync(protocol, next.value.resolution)
    } else {
      value = await listAsync(protocol, next.value.prefix)
    }
...
function resolveSync(protocol, url) {
  const resolution = protocol.resolve(url)

  if (isThenable(resolution)) {
    throw errors.ASYNCHRONOUS_READ('Protocol resolve returned a promise during synchronous linking')
  }

  return resolution
}

function resolveAsync(protocol, url) {
  return protocol.resolve(url)
}

function readSync(protocol, url) {
  if (existsSync(protocol, url) === false) return null

  const source = protocol.read(url)

  if (isThenable(source)) {
    throw errors.ASYNCHRONOUS_READ('Protocol read returned a promise during synchronous linking')
  }

  return source
}

async function readAsync(protocol, url) {
  if ((await existsAsync(protocol, url)) === false) return null

  return protocol.read(url)
}

function existsSync(protocol, url) {
  const exists = protocol.exists(url)

  if (isThenable(exists)) {
    throw errors.ASYNCHRONOUS_READ('Protocol exists returned a promise during synchronous linking')
  }

  return exists
}

function existsAsync(protocol, url) {
...
function listSync(protocol, url) {
  ...
  for (const found of protocol.list(url)) urls.push(found)
