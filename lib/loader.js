...
module.exports = class ModuleLoader {
  constructor(opts = {}) {
    ...
    this._protocol = protocol === null ? new ModuleProtocol() : assertProtocol(protocol)
  ...
  _protocolFor(url) {
    ...
    return this._protocol.extend({
      ...
      exists(context, url) {
        if (bundleOwns(bundle, url)) return true

        const exists = context.exists(url)

        if (isThenable(exists)) return exists.then((found) => found || bundle.exists(url.href))

        return exists || bundle.exists(url.href)
      },

      existsSync(context, url) {
        if (bundleOwns(bundle, url)) return true

        return context.existsSync(url) || bundle.exists(url.href)
      },

      read(context, url) {
        if (bundleOwns(bundle, url)) return bundle.read(url.href)

        const source = context.read(url)

        if (isThenable(source)) return source.then((found) => found || bundle.read(url.href))

        return source || bundle.read(url.href)
      },

      readSync(context, url) {
        if (bundleOwns(bundle, url)) return bundle.read(url.href)

        return context.readSync(url) || bundle.read(url.href)
      },
  ...
  *_drive(generator, onChild) {
  ...
  _linkBundle(bundle) {
    ...
    const protocol = this._protocol.extend({
      exists(context, url) {
        ...
        if (isThenable(exists)) return bundle.exists(url.href)
      ...
      read(context, url) {
        ...
        if (isThenable(source)) return bundle.read(url.href)
      ...
      readSync(context, url) {
        ...
        const source = context.readSync(url)
  ...
  _assertType(record, attributes) {
    ...
    const expected = typeByAttribute[requested]
    ...
    if (expected !== actual) {
      throw errors.TYPE_INCOMPATIBLE(`Module '${record.url.href}' is not of type '${requested}'`)
    }
  }
...
function notFound(condition, specifier, parentURL, candidates = []) {
  switch (condition) {
    case 'addon':
      return errors.ADDON_NOT_FOUND(
        `Cannot find addon '${specifier}'`,
        specifier,
        parentURL,
        candidates
      )
...
function bundleOwns(bundle, url) {
  ...
  if (typeof root === 'string' && url.href.startsWith(root) === false) return false
