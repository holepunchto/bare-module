...
module.exports = class ModuleLoader {
  constructor(opts = {}) {
    ...
    // handle to the loader that read it. Reusing one across protocols would
    // hand the modules of one graph to a loader that may not read as far.
    for (const href in this._cache) {
      const record = this._cache[href]

      if (record && record._loader._protocol !== this._protocol) {
        throw errors.PROTOCOL_MISMATCH(
  ...
  _addonRecord(url) {
    ...
    addURL(this._artifacts.addons, url)
...
function addURL(urls, url) {
  let lo = 0
  let hi = urls.length - 1

  while (lo <= hi) {
    const mid = lo + ((hi - lo) >> 1)
    const found = urls[mid]

    if (found.href === url.href) return

    if (found.href < url.href) lo = mid + 1
    else hi = mid - 1
  }

  urls.splice(lo, 0, url)
}
