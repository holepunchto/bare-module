  ...
  async _resolveAsync(specifier, parentURL, condition) {
    ...
    while (next.done !== true) {
      ...
      if (value.package) {
        ...
        let exists = existsAsync(protocol, url)

        if (isThenable(exists)) exists = await exists

        if (exists) {
          let resolution = resolveAsync(protocol, url)
...
async function driveAsync(generator, protocol) {
  ...
  while (next.done !== true) {
    ...
    if ('module' in request) {
      value = readAsync(protocol, request.module)
    } else if ('probe' in request) {
      value = existsAsync(protocol, request.probe)
    } else if ('resolution' in request) {
      value = resolveAsync(protocol, request.resolution)
    } else {
      value = listAsync(protocol, request.prefix)
    }

    next = generator.next(isThenable(value) ? await value : value)
  }
...
function readAsync(protocol, url) {
  const exists = existsAsync(protocol, url)
